...
class OfflineBuffer:
    ...
    def __init__(
        ...
        self._extra_data: dict[str, np.ndarray] = {}
        self._size = 0  
    ...
    def load_zarr(self, zarr_path: str) -> None:
        ...
        dataset = {
            key: np.asarray(data_group[key][:])
            for key in data_group.keys()
            if self._use_depth or key not in depth_keys
        }
        dataset["episode_ends"] = np.asarray(meta_group["episode_ends"][:],dtype=np.int64,)
    ...
    def load_dataset(self, dataset: dict[str, np.ndarray]) -> None:
        ...
        reserved = {"state", "action", "reward", "next_state", "next_action","done", "timeout", "episode_ends", "not_done", "return",}
    ...
    def _validate_dataset(self, dataset: dict[str, np.ndarray]) -> None:
        core_keys = (
            "state", "action", "reward", "next_state", "next_action",
            "done", "timeout", "episode_ends",
        )

        rgb_keys = self.RGB_KEYS + tuple(
        ...
        if self._use_depth:
            depth_keys = self.DEPTH_KEYS + tuple(
                f"next_{key}" for key in self.DEPTH_KEYS
            )
            required += depth_keys
